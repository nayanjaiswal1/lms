...
import { AssessmentSearchInput } from "@/app/(app)/assessments/manage/assessment-search-input";
import { AssessmentCardMenu } from "@/app/(app)/assessments/manage/assessment-card-menu";
  ...
  return (
    <article className="card-interactive flex flex-col gap-3 p-6">
      <div className="flex items-start justify-between gap-2">
        <Link className="text-base font-semibold hover:underline" href={ROUTES.manageAssessment(a.id)}>
          {a.title}
        </Link>
      ...
      <div className="mt-auto flex gap-2">
        <Button asChild size="sm" variant="outline">
          <Link href={ROUTES.manageAssessment(a.id)}>Manage questions</Link>
        </Button>
        <Button asChild size="sm" variant="ghost">
          <Link href={ROUTES.manageAssessmentResults(a.id)}>
            <BarChart3 /> Results
          </Link>
        </Button>
      </div>
