...
// The real course/roadmap/batch link is course_modules.assessment_id (see the
// courseLinkJoin comment in repo_assessment.go) — parent_type/parent_id on the
// assessment row are cosmetic and the backend nulls parent_id automatically
// for standalone/hiring, so switching scope here is safe for any assessment.
function toScopeValue(parentType: string): AssessmentParentType {
  return parentType === ASSESSMENT_PARENT_TYPE.HIRING
    ? ASSESSMENT_PARENT_TYPE.HIRING
    : ASSESSMENT_PARENT_TYPE.STANDALONE;
}
...
export function EditAssessmentSettingsForm({ assessment }: EditAssessmentSettingsFormProps) {
  ...
  const [parentType, setParentType] = useState<AssessmentParentType>(toScopeValue(assessment.parent_type));
