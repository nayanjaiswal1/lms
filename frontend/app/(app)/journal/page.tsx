...
interface JournalPageProps {
  searchParams: Promise<{ category?: string; subcategory?: string; q?: string; limit?: string }>;
}

export default async function JournalPage({ searchParams }: JournalPageProps) {
  const { category, subcategory, q, limit: limitParam } = await searchParams;
  const limit = Number(limitParam) || DEFAULT_LIMIT;
  const [entries, categories] = await Promise.all([
    getJournalEntries({ category, subcategory, search: q, limit }),
