...
export function PlanningBottomNav() {
  ...
  return (
    <nav
      ...
      className="safe-bottom fixed inset-x-0 bottom-0 z-sticky flex items-center justify-around border-t border-border/80 bg-card/95 px-2 py-1.5 shadow-modal backdrop-blur-md lg:hidden"
    >
      {PLANNING_BOTTOM_NAV.map(({ label, href, icon: Icon }) => {
        ...
        return (
          ...
          >
            ...
            <span className={cn("mt-0.5 text-xs", active ? "font-bold" : "font-medium")}>{label}</span>
          </Link>
        );
      })}
    </nav>
