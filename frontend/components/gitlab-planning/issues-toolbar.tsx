...
import type { AeIssuesPage } from "@/lib/gitlab-planning/types";
    ...
    <>
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="m-label-sm flex items-center gap-1 rounded-lg bg-(--m-sc-low) px-2 py-0.5 text-(--m-on-surface-variant)">
            <RefreshCw className="size-3.5 text-(--m-tertiary)" aria-hidden />
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Link href={href({ ...keep, bulk: bulk ? "0" : undefined })} scroll={false} className={GHOST_BTN} aria-pressed={bulk}>
            <ListChecks className="size-4" aria-hidden />
            <span>Bulk edit</span>
          </Link>
          <button type="button" className="m-headline-sm flex h-8 items-center gap-1 rounded-lg bg-(--m-primary) px-3 text-(--m-sc-lowest) shadow-sm transition-colors hover:bg-(--m-primary-container)">
      ...
      <div className="flex items-center justify-between rounded-xl bg-(--m-sc-lowest) px-2 shadow-sm">
        ...
        <div className="hidden items-center gap-1 py-1 sm:flex">
          ...
          <button type="button" aria-label="RSS feed" className="rounded-lg p-1 text-(--m-on-surface-variant) hover:bg-(--m-sc-low) hover:text-(--m-on-surface)">
            <Rss className="size-4.5" aria-hidden />
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5 rounded-xl bg-(--m-sc-lowest) p-2 shadow-sm">
        <div className="flex flex-col items-stretch gap-1.5 lg:flex-row lg:items-center">
          <div className="flex flex-1 flex-wrap items-center gap-1 rounded-lg bg-(--m-sc-low) px-2 py-1.5 transition-colors focus-within:bg-(--m-sc)">
            <ListFilter className="size-4.5 shrink-0 text-(--m-on-surface-variant)" aria-hidden />
            {page.filters.map((f) => (
              <span key={f.key} data-mtone={f.tone} className="m-label-sm flex items-center gap-1 rounded-lg bg-(--m-sc-lowest) px-2 py-0.5 text-(--m-on-surface) shadow-2xs">
                ...
                <button type="button" aria-label={`Remove ${f.key} filter`} className="ml-0.5 flex items-center text-(--m-on-surface-variant) hover:text-(--m-error)">
            ...
            />
            <button type="button" className="m-label-sm rounded px-1.5 py-0.5 text-[11px] text-(--m-on-surface-variant) hover:bg-(--m-sc-highest) hover:text-(--m-on-surface)">
              Clear all
            </button>
          </div>
          <div className="flex shrink-0 items-center gap-1 self-end lg:self-auto">
            <label className="flex h-9 items-center rounded-lg bg-(--m-sc-low) px-2 py-1.5">
              <ArrowUpDown className="mr-1 size-4 text-(--m-on-surface-variant)" aria-hidden />
            ...
            <button type="button" aria-label="Reverse sort direction" className="flex size-9 items-center justify-center rounded-lg bg-(--m-sc-low) text-(--m-on-surface-variant) transition-colors hover:bg-(--m-sc) hover:text-(--m-on-surface)">
        ...
        <div className="flex flex-wrap items-center gap-1 pt-1">
          ...
          {page.quick_filters.map((q) => (
            <button key={q.label} type="button" className="m-label-sm flex items-center gap-1 rounded-lg bg-(--m-sc-low) px-2.5 py-1 text-(--m-on-surface-variant) transition-colors hover:bg-(--m-sc) hover:text-(--m-on-surface)">
              {q.dot && <span data-mtone={q.dot} className="size-1.5 rounded-full bg-(--mc)" />}
              <span>{q.label}</span>
