...
export function ModuleNavFooter({ courseSlug, prevModule, nextModule }: ModuleNavFooterProps) {
  if (!prevModule && !nextModule) return null;

  return (
    <nav
      aria-label="Module navigation"
      className="mt-10 flex divide-x divide-border overflow-hidden rounded-xl border border-border bg-card shadow-card"
    >
      {prevModule ? (
        <Link
