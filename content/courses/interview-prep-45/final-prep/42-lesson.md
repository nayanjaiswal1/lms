...
3. Name the ONE hardest part of the system and say in two sentences how you'd approach it, without actually solving it (2 minutes).
...
**Verify you're actually strong here:** if you needed the full 20 minutes for an "easy" problem, or you hesitated more than 5 seconds on where to start a design, that's signal worth noting, not something to fix today, just something to be aware of walking in.
...
| Action | What YOU did, step by step | Using "we" throughout, which leaves interviewers unable to score a team's actions against you |
...
- *Time complexity of building a heap from an array?* O(n), not O(n log n), because most nodes are near the bottom and sift down a short distance (amortized analysis, not a naive per-node bound).
...
**Verify you're actually strong here:** if you paused more than 3-4 seconds on any answer above, that's not a gap to close today, just a mental note as a likely follow-up question to expect.
...
- **Light movement.** A walk or stretching lowers cortisol better than sitting with your notes one more time.
...
The whole day runs on one idea: everything you needed to learn is already learned, and tonight's only job is to protect it. Chasing one more gap costs more in sleep than it returns in coverage.
