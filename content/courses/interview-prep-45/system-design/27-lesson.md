...
- A local agent that buffers and ships asynchronously, plus a Kafka-style ingestion buffer decoupling accept-from-process, is what lets this system absorb the traffic spikes that incidents themselves cause, without falling over exactly when it's needed most. That's the system's most important non-functional requirement, and it repeats the "separate write-optimized ingestion from read-optimized index" pattern you've now seen in Twitter search and e-commerce catalog search too.
