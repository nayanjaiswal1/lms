...
- Each search query averages ~20 characters typed, so roughly 20 autocomplete requests per search (one per keystroke, though clients typically debounce/throttle this).
...
- Update volume: new/incremented queries per day, roughly 500M events, need to feed back into rankings, but this can be batched (e.g., hourly) rather than applied per-query in real time.
...
**Trie (in-memory serving structure):** each node represents one character, and each node caches its own top-K most frequent completions so a lookup doesn't need to walk the full subtree at request time.
