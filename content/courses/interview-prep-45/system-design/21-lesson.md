...
- WhatsApp: message delivery needs at-least-once guarantees (never silently drop a message) with client-side deduplication (message IDs), ordering per-conversation (not globally), and — distinctively — end-to-end encryption, meaning the server relays ciphertext it cannot read, which constrains server-side features (no server-side search/indexing of message content, message queuing for offline recipients still keyed by opaque IDs).
...
- This week's unifying theme is contention and coordination (spatial matching, concurrent editing, booking races, message ordering) versus last week's read-heavy/caching theme — notice which theme a new prompt fits before picking your toolkit.
- Geospatial indexes (geohash/quadtree/R-tree) are the named answer for any "nearest" or "within an area" query — say the structure by name.
- OT and CRDTs solve the concurrent-merge problem specifically; don't conflate them with the transport layer (WebSockets) that just moves the operations around.
