...
- Posts/day: 200M users, 5% post daily → 10M posts/day ≈ 116 posts/sec average, ~350/sec peak.
- Feed reads/day: each DAU opens the feed ~10 times/day → 2B feed reads/day ≈ 23,000 reads/sec average, ~70,000/sec peak.
...
Cursor-based pagination (not offset) — offset pagination degrades badly (`OFFSET 50000` scans and discards 50,000 rows) and breaks under concurrent inserts (items shift between pages). Cursor = the last seen post's `(created_at, post_id)` tuple, encoded opaquely.
...
- *Hybrid (what production systems actually do)*: fan-out on write for normal users; for celebrity/high-follower accounts (say, >100K followers), skip the fan-out — pushing one post to 50M feed_items rows would be a write storm — and instead merge their posts into the feed at read time. The read path becomes: fetch precomputed feed_items + separately query "any new posts from the celebrities I follow" + merge by timestamp.
...
**Cache invalidation for likes/comments.** Counts are hot and mutate constantly — store them denormalized on the post (`like_count` column) updated via an async counter service (Redis `INCR`, periodically flushed to the DB) rather than `COUNT(*)` on every read.
...
| Consistency | Eventual (feed lag of seconds is fine) | A "like" might briefly show a stale count — acceptable trade for availability |
...
A: Instrument the fan-out worker with delivery tracking — an event log of "post X should reach N followers" vs "post X was pushed to feed_items for N' followers" and alert on divergence. Add idempotent fan-out (dedup by post_id+follower_id) so retries after partial failure don't double-insert, and a reconciliation job that periodically re-derives a sample of feeds from source-of-truth follow + post data to catch drift.

## Key takeaways

- The read:write ratio (often 100-1000:1 for feeds) is the number that justifies fan-out-on-write and aggressive caching — say it explicitly in the interview.
- Fan-out on write trades write cost (rare) for read speed (frequent) — the right trade for feeds.
- The celebrity problem forces a hybrid: fan-out on write for normal accounts, read-time merge for high-follower accounts.
