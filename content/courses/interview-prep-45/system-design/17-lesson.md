...
- Average active editing session generates ~1 keystroke/edit-op every ~0.5-1s while typing → 5M sessions × ~1.5 ops/sec (accounting for idle time) ≈ **~1-2M ops/sec system-wide at peak** — but concentrated per-document: a single popular shared doc with 50 concurrent editors sees maybe 50-100 ops/sec, which is the number that actually matters for per-document server capacity.
- Each edit op is tiny (~50-200 bytes: op type, position, content delta, revision number) → the wire cost of a keystroke is trivial; the hard part is ordering and merging, not bandwidth.
- Document storage: average doc ≈ 50 KB of content + operation history. Storing every historical op forever for 1B docs is expensive, so history is periodically compacted into snapshots (e.g., keep full op log for 30 days, then collapse to periodic snapshots).
...
- Central-server OT (what Google Docs actually uses) is simpler to reason about than CRDTs when you already need a central authority for access control and presence.
- Single-writer-per-document (via consistent hashing) gives you a natural ordering point without full distributed consensus per keystroke.
