...
- Cart storage optimizes for availability and low latency (Redis-backed, async durable sync); checkout optimizes for strict correctness (explicit state machine, atomic reservation, idempotency) — know which guarantee applies to which layer and why they differ.
- Checkout is a multi-step state machine, not a single transaction, specifically because it must coordinate with an external payment provider that a DB transaction can't roll back.
