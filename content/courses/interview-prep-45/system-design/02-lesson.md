...
A rate limiter caps how many requests a client (per user, API key, or IP) can make in a time window, protecting backend services from abuse, runaway retries, and cost blowouts. It typically sits at the API gateway or edge, before requests reach application servers.
...
### Non-functional
- **Accuracy vs performance trade-off.** Perfectly precise limiting requires coordination, which costs latency; approximate limiting is faster but allows some burst-over.
...
- **Cons:** the boundary burst problem. A client can send N requests at 0:59 and another N at 1:00, getting 2N requests in 2 seconds around the window edge.

### 2. Sliding window log
Store a timestamp for every request in a sorted set (e.g. Redis ZSET). On each request, drop timestamps older than `now - window`, count what's left, and allow or reject.
...
- **Cons:** memory grows with request volume (must store every timestamp), and each check is more expensive (O(log n) or a range scan).
...
- **Cons:** it's an approximation, not exact, though that's acceptable for almost all real-world use cases. This is the industry-favored answer (used by Cloudflare, Stripe-style gateways) because it balances accuracy and cost.
...
- **Pros:** naturally allows bursts up to bucket capacity while enforcing a long-term average rate, which matches how APIs actually want to behave: burst is fine, sustained abuse isn't.
- **Cons:** slightly more state to track (tokens plus last-refill timestamp) than a fixed counter.
...
- **Mental model:** a queue draining at constant speed. It tracks how backed up you are, not how much credit is left.
...
- **Cons:** legitimate bursts get delayed or queued rather than allowed, which can hurt UX for spiky-but-legitimate clients.

> Token bucket and leaky bucket are mirror images of the same idea. One grants credit that drains over time; the other drains a fixed-rate queue, running in the opposite direction. If you can explain one, you can derive the other on the whiteboard.
...
**Answer:** in-memory only works if you pin a client to one gateway instance (sticky routing), which is fragile and doesn't survive scaling events. For a real distributed API gateway, use Redis: store counters as `INCR key` with `EXPIRE`, or use a Lua script to make check-and-increment atomic in one round trip. That avoids a race between two concurrent requests both reading "under limit" before either increments.

The exact Redis primitive follows the algorithm you picked:
- **Fixed window:** `INCR ratelimit:{key}:{window}`, with `EXPIRE` set only on the first increment of a window. The key self-cleans via TTL, no separate cleanup job needed.
- **Sliding window log:** `ZADD` the current timestamp into a per-key sorted set, `ZREMRANGEBYSCORE` to prune anything older than `now - window`, then `ZCARD` to get the count. Wrap prune, count, and add in one Lua script or `MULTI`/`EXEC` pipeline so nothing interleaves.
- **Sliding window counter:** two keys, one per window (`ratelimit:{key}:{window_n}` and `ratelimit:{key}:{window_n-1}`), each a plain `INCR`. On read, fetch both and apply the weighted-average formula above. Cheap, and no Lua required for the read path.
...
The rate limiter is middleware, not a separate hop. It runs inline in the gateway request path, calling Redis synchronously before forwarding. Redis is the shared state store; use `EXPIRE`/TTL so keys self-clean and memory doesn't grow unbounded.
...
Redis Cluster shards counters by key hash, which scales horizontally, but a single very hot key (one abusive client) still hits one shard. That's usually fine since it's a single client's traffic, not global traffic.

Consider a local in-memory "fast reject" cache: if a client is already known to be way over limit, reject without even calling Redis, saving a network hop for the abuse case. And push rate-limit headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`) so well-behaved clients can self-throttle.
