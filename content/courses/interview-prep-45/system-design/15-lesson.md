...
- 20M daily active riders, 1M active drivers, 100 major metros.
- Average 10M trips/day → ~115 trips/second average, ~5-8x that at peak (rush hour) → ~700-900 trips/sec peak.
- Location pings: 1M drivers online at peak, each pinging every 4 seconds → 1,000,000 / 4 ≈ **250,000 location writes/sec** at peak. This is the dominant write load, not trip creation.
- Each location ping is small (~100 bytes: driver_id, lat, lng, heading, timestamp) → 250,000 × 100 bytes ≈ 25 MB/sec ingest.
...
- **Location Service** ingests the 250K/sec location stream. Writes fan out to (a) an in-memory geospatial index for matching queries and (b) a lightweight time-series store for "replay the last N minutes" (used by support and fraud tooling). Nothing here touches the durable trip database.
...
- Split the system by access pattern: location (huge write volume, ephemeral, in-memory) vs. trip/payment (moderate volume, durable, strongly consistent).
- Geospatial indexing (geohash/S2 cells) turns "nearest driver" from an O(n) scan into a cell lookup — this is the single highest-leverage design decision in the whole system.
