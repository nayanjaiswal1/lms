...
Average track ≈ 3.5 minutes, so ~17 track-plays per user per day. 200M × 17 ≈ **3.4B track starts/day**, or ~40,000 track starts/sec average, several times that at evening peak.

Audio file size: a 3.5-minute track encoded at 128kbps ≈ 3.5 × 60 × 128 kbit / 8 ≈ 3.4 MB per quality tier. Store 3 tiers (e.g., 96/160/320 kbps) for ~10 MB per track across tiers.
...
Streaming bandwidth: 200M DAU × 1 hour/day × ~1 Mbps average stream rate is an enormous aggregate bandwidth, almost entirely absorbed by CDN edge caches, since a small fraction of tracks (the "hot" catalog) accounts for the majority of plays (a classic Zipfian distribution).
