...
The URL shortener is the "FizzBuzz" of system design interviews — deliberately simple so the interviewer can watch how you structure an ambiguous problem: how you scope requirements, do back-of-envelope math, pick a key-generation strategy, and reason about read-heavy traffic. If you fumble this one, harder problems (feed, chat, Netflix) are off the table. Treat it as a chance to demonstrate a repeatable framework, not to memorize a diagram.
...
### Non-functional
- **High availability** — a broken redirect service breaks every link ever shared; uptime matters more than perfect consistency.
...
- **Writes/sec:** 100,000,000 / (30 × 24 × 3600) ≈ **38 writes/sec** average; design for 5-10x peak ≈ 200-400/sec.
- **Reads/sec:** 38 × 100 ≈ **3,800 redirects/sec** average, peak maybe 20,000/sec.
- **Storage:** each row ~ 500 bytes (long URL + code + metadata). 100M/month × 5 years × 12 × 500 B ≈ 100M × 60 × 500 B ≈ **3 TB** over 5 years — small enough to shard lightly or even keep in one well-indexed table with read replicas.
...
301 vs 302: **302 (temporary)** is usually correct for a shortener — it stops browsers from caching the redirect forever, which keeps click analytics accurate and lets you change/expire the destination later. 301 saves origin load (browser caches it) but you lose click tracking and flexibility.
...
Keep `click_count` as an eventually-consistent counter (increment async via a queue or a periodic batch job) rather than an update-per-redirect — a synchronous `UPDATE` on every redirect turns your hot read path into a hot write path and creates row-lock contention on popular links.
...
- **Write path:** client submits a long URL → service generates/validates a short code → writes to primary DB → returns the short URL.
- **Read path:** redirect requests hit the load balancer → check cache first → on hit, redirect immediately; on miss, read from a DB replica, populate cache, then redirect.
...
- Cons: sequential/incrementing IDs are guessable/enumerable unless you shuffle or offset them (e.g. XOR with a secret, or use Snowflake IDs which interleave timestamp + machine ID + sequence to avoid a single point of ID allocation).

**Interview-favored answer:** pre-generate a pool of unique keys offline (a **Key Generation Service**, KGS) using an ID sequence, hand blocks of keys (e.g. 1,000 at a time) out to app servers so writes never contend on a single counter, and mark keys "used" only after successful insert. This removes the collision-check round trip from the write path entirely and avoids a single hot sequence.

### Custom aliases

Custom aliases go through the same `short_code` column but skip key generation — just check availability with a unique constraint (`INSERT ... ON CONFLICT DO NOTHING`, return conflict error if taken).
...
- **Database choice:** a relational DB (Postgres/MySQL) is fine up to a very large scale because access is by primary key (`short_code`) — no complex joins. Move to horizontal sharding (by hash of `short_code`) only once a single primary can't keep up with write throughput or storage.
- **Read replicas** absorb the 100:1 read-heavy load cheaply; the cache absorbs even more.
- **Cache eviction:** LRU is fine — cold, rarely-clicked links naturally fall out and re-populate from DB on next access.
...
- **Consistency vs availability:** favor availability — a slightly stale click count or a few-second delay in a new link being cache-visible is acceptable; a failed redirect is not.
...
A: Store `expires_at`, check it on cache miss during read (lazy expiration), and run a low-priority background job that periodically deletes expired rows in batches (active expiration) so storage doesn't grow unbounded — same pattern Redis itself uses internally.

## Key takeaways
- Read:write ratio (~100:1) should drive every design decision here — optimize the read path aggressively.
- Base-62 encoding of a uniquely-generated ID beats random-string-plus-collision-check at scale.
