...
A notification service is a canonical asynchronous, multi-provider fan-out system. It tests whether you reach for a message queue instead of synchronous calls, whether you understand at-least-once delivery and idempotency, and whether you can design retry/backoff logic that doesn't hammer a failing downstream provider. Almost every real backend eventually needs this — expect it in mid-to-senior interviews.
...
- **Average rate:** 10,000,000 / 86,400 ≈ **116/sec**; peak (e.g. a marketing blast) could be 10-50x ≈ 1,000-5,000/sec.
- **Storage:** notification record ~1 KB (recipient, channel, template, status, timestamps) × 10M/day × 365 days ≈ 3.65B rows/year, ~3.65 TB/year raw — plan to archive/cold-store records older than a few months to a cheaper store (S3 + occasional query via Athena) rather than keeping everything in the hot DB.
- **Provider rate limits:** e.g. an SMS gateway might allow 100 msgs/sec — this becomes the bottleneck, not your own infrastructure, so the queue must be able to buffer bursts and drain at the provider's allowed rate.
...
- **Notification API** validates the request, checks user preferences, resolves the template, writes a `queued` row, and publishes an event to the queue. It never calls the provider synchronously — this decouples "accept the request" from "actually deliver it."
...
Kafka or Kinesis if you need replay, multiple independent consumers, and high throughput with ordering guarantees per partition key. SQS (standard or FIFO) is simpler operationally if you don't need replay — SQS FIFO gives per-`MessageGroupId` ordering, mapping naturally to per-user ordering. For an interview, either is acceptable — justify the pick based on whether you need replay/multiple consumer groups (Kafka) or just simple reliable delivery (SQS).

### Templates

Store templates with versioned placeholders (`{{order_id}}`), render server-side at send time (not at trigger time) so the latest template/branding is always used even if a message sits in the queue for a while. Support per-locale template variants (`shipped_en`, `shipped_es`) keyed by user's locale preference.

### Retry logic and dead-letter queue

- On provider failure, retry with **exponential backoff + jitter** (e.g. 1s, 2s, 4s, 8s, capped, plus random jitter to avoid thundering-herd retries all synchronized to the same instant).
- Cap retries (e.g. 5 attempts) — after that, move to a **dead-letter queue (DLQ)** for manual inspection/alerting rather than retrying forever.
...
- Track `attempt_count` and `last_error` on the notification row for observability.
- Provider delivery webhooks (SES bounce/complaint, Twilio delivery receipt) update status asynchronously after the initial "sent" state — "sent" (we handed it to the provider) is different from "delivered" (the provider confirms the recipient's device/inbox got it).
- Respect quiet hours and opt-outs before even enqueueing — cheaper than enqueueing and discarding.

## Scaling & trade-offs

- **At-least-once delivery is the realistic guarantee** — exactly-once across a network boundary to a third-party provider is not achievable in practice. Use the `idempotency_key` so retries (ours or the caller's) don't create duplicate sends.
- **Ordering vs throughput:** strict per-user ordering caps parallelism to one-in-flight-message-per-user; that's fine because per-user notification volume is naturally low, so it doesn't bottleneck overall throughput.
- **Provider fan-out for reliability:** support a primary + fallback provider per channel (e.g. SendGrid primary, SES fallback) and fail over automatically if the primary's error rate spikes, using a circuit breaker.
...
A: Idempotency key at the API layer (dedupe on insert) plus idempotency at the worker layer — check the notification's current status before sending; if already `sent`/`delivered`, skip. This makes retries (queue redelivery, our own retry logic) safe.
...
A: Add a rate-limit/dedupe layer keyed by `(user_id, notification_type)` with a TTL matching the desired window (e.g. Redis key with 24h TTL) — check it before enqueueing, and only enqueue if the key doesn't already exist.

## Key takeaways
- Decouple "accept the trigger" from "deliver the message" with a queue — never call the provider synchronously from the API request path.
- Partition the queue by user ID to get natural per-user ordering while still parallelizing across users.
