...
Pattern coverage checklist, marked honestly and not aspirationally:
...
- **FastAPI:** `Depends()` injects DB sessions, auth, and pagination so route handlers stay thin and testable; `async def` route handlers only help when everything they await is actually non-blocking, since a synchronous ORM call inside an async handler blocks the event loop.
- **PostgreSQL:** run `EXPLAIN ANALYZE` before assuming an index will help. A sequential scan can beat an index scan on a small table or a low-selectivity column; composite indexes are ordered, so column order matters for which queries they actually serve.
...
- **React:** the dependency array in `useEffect` is a correctness mechanism, not a performance knob. A missing dependency means the closure captured a stale value, which is a bug, not merely "one extra render."
...
- [ ] System design once weekly, rotating through systems you haven't touched recently rather than the ones you're already confident in.
- [ ] Mock interview monthly, with a real person rather than self-review alone. Self-review misses delivery problems the same way reading a script hides filler words.
...
The pattern running through every section above is the same: a rating only counts once it survives a cold, timed check, and a weak spot only matters once it has an actual date on the calendar to get fixed. That's the difference between this audit and just feeling generally ready.
