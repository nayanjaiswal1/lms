...
Recursion *is* an implicit stack — each recursive call pushes a new frame (local variables, return address) onto the call stack, and returning pops it. Any recursive algorithm can be rewritten iteratively using an explicit stack; this matters in interviews because:
...
**Approach:** Push opening brackets. On a closing bracket, pop and check it matches the expected opener; mismatch or empty stack means invalid. String is valid only if the stack is empty at the end.
...
**Common mistakes:**
- Brute-forcing with a nested loop (O(n²)) — works but is the naive baseline interviewers expect you to improve on.
- Storing values instead of indices on the stack — you need the index to compute the distance, not just the temperature.
...
**Intuition:** A normal stack gives O(1) push/pop/top but O(n) minimum (you'd have to scan). Track the running minimum *alongside* each element so every push carries "what was the min including me," giving O(1) min retrieval too.

**Approach:** Use two parallel stacks — one for values, one for the minimum-so-far at each depth (or store `(value, current_min)` tuples in a single stack).
...
- Stacks give O(1) push/pop from one end only — use `list.append`/`list.pop()` in Python, never index-0 operations.
- Any recursive algorithm can become iterative using an explicit stack; this avoids recursion-depth limits and shows you understand the call stack.
