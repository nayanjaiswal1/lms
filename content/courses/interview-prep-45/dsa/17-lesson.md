...
The pattern: place one queen per row (this eliminates row conflicts by construction, so you never need to check rows), try every column in that row, and recurse to the next row only if the placement is safe. Backtrack (remove the queen, unmark the sets) when a branch is exhausted.
...
Sudoku is N-Queens's constraint-satisfaction cousin with more constraint types: each number must be unique in its row, column, *and* 3x3 box. The backtracking shape is identical (try a value, recurse, undo); what changes is that the validity check needs three simultaneous conditions instead of one.
...
**Common mistakes:** using `row + col` and `row - col` backwards or checking only one diagonal; rebuilding the board string on every recursive call instead of only at `row == n`; forgetting to remove from all three sets on backtrack, since a partial undo corrupts every sibling branch after the first.
...
**Common mistakes:** building the full board anyway out of habit, wasting work when only a count is needed. Recognizing "I don't need the reconstruction step" is itself an interview signal.
...
**Intuition:** Each digit maps to a fixed set of letters (like an old T9 keypad). The output is the Cartesian product of each digit's letter set, and backtracking naturally enumerates a Cartesian product by looping over one dimension's options per recursion level.
...
**Common mistakes:** trying to solve this iteratively with nested loops for a variable number of digits (works but is far messier than recursion since digit count isn't fixed); forgetting the `if not digits: return []` edge case, since LeetCode expects an empty list, not `[""]`.
...
N-Queens, Sudoku, and Restore IP Addresses look unrelated at first glance, but they're the same backtracking template wearing different validity checks: precomputed sets for N-Queens, three simultaneous scans for Sudoku, segment-length arithmetic for IP addresses. Once you see the validity check as a pluggable piece, adapting the template to a new CSP is mostly a matter of asking what "invalid" means for this problem and how cheaply you can detect it.
