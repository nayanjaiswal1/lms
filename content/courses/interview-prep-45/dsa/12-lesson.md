...
Dynamic programming trips up more candidates than any other topic, because finding the state is often harder than writing the code once you have it. Today you build the mental model, recursion to memoization to tabulation, on the simplest possible problems, so the pattern is automatic before you hit harder DP later in the course.

## Recursion vs memoization vs tabulation

**Plain recursion** re-solves the same subproblem repeatedly when subproblems overlap. That's what makes naive recursive solutions exponential.
...
**Workflow in an interview:** write the naive recursive solution first, even just mentally, to nail the recurrence. Add memoization to show you understand the overlap. Then offer tabulation, and space optimization, as the polish. The progression itself demonstrates DP fluency to the interviewer.
...
- Two-string problems (tomorrow): state = `(i, j)`, the position in *each* string.

If two different state definitions give different answers to the same subproblem, the state is incomplete. You're missing a dimension, such as a boolean for "have I used my one free skip yet."
...
Get the base cases wrong, and every downstream value is wrong even if the transition logic is perfect. Always verify base cases against the smallest one or two concrete examples by hand before coding.
...
**Common mistakes:** Off-by-one on base cases; `dp[0]` should be 1, not 0, since there's exactly one way to be at the ground with zero steps taken (do nothing). Recomputing with unmemoized recursion, giving O(2^n).
...
**Approach:** `dp[i] = max(dp[i-1], dp[i-2] + nums[i])`. Either skip house `i` and carry forward `dp[i-1]`, or rob it: `nums[i]` plus the best from two houses back, since the adjacent one is now off-limits.
...
**Common mistakes:** Trying to track "which houses were robbed" instead of just the max value. The problem only asks for the amount, so the state is simpler than it first appears. Confusing this with a greedy "always take the bigger neighbor" approach, which is wrong: DP correctly looks ahead via the recurrence, greedy doesn't.

## House Robber II

[LeetCode 213](https://leetcode.com/problems/house-robber-ii/), DP, Circular array

**Intuition:** Houses are arranged in a circle, so house 0 and house `n-1` are now adjacent too, and you can't rob both. Split into two cases: rob from the range excluding the last house, or rob from the range excluding the first house. The answer is the max of the two, each solved with plain House Robber.

**Approach:** Run the House Robber I logic twice, once on `nums[0:n-1]` and once on `nums[1:n]`, and take the max. Handle `n == 1` as a special case, since a single house has no "circle" conflict.
