...
---
Arrays and hash tables are the substrate almost every other pattern in this course builds on — two pointers, sliding window, and even graph adjacency lists are arrays and hash maps in disguise. Interviewers use these three problems as a calibration check: if you fumble a hash-map complement lookup, they downgrade their expectations for everything harder that follows. Today you get the mental model right once so it stops costing you thinking time later.
...
- Resizing is O(n) but amortized across n inserts it's O(1) per insert — same argument as dynamic array growth.
...
**Why append is amortized O(1):** a resize costs O(n) to copy, but doubling means resizes happen at sizes 1, 2, 4, 8, ... 2^k. Summing the copy costs (1 + 2 + 4 + ... + n ≈ 2n) and dividing by n appends gives O(1) average cost per append. Growing by a *fixed* increment instead of doubling makes append O(n) amortized — that's the detail interviewers probe for.

## Space complexity trade-offs

Space complexity analysis asks: how does auxiliary memory (not counting the input) grow with input size n? The classic trade-off in this unit is **time vs. space via hashing**: a hash set turns an O(n) "does this exist" scan into O(1) lookup, at the cost of O(n) extra memory to store the set.
...
**Intuition:** Brute force checks every pair — O(n²). But for each number `x`, you only need to know: has its complement `target - x` appeared already? That's an existence + lookup question, which is exactly what a hash map answers in O(1).

**Approach:** Walk the array once. For each element, compute the complement. If the complement is already in the map, you've found your pair — return the stored index and the current index. Otherwise, store the current element's value → index and keep going.
...
**Complexity:** Time O(n) — one pass, O(1) map operations. Space O(n) for the map.
...
**Complexity:** Time O(n) where n is string length. Space O(k) where k is the alphabet size (O(1) if you assume fixed alphabet like lowercase ASCII).

**Common mistakes:**
- Sorting both strings and comparing — works (O(n log n)) but is strictly worse than counting and interviewers will ask "can you do better?"
- Forgetting the early length check, which is a cheap win.
...
A one-liner alternative trades early exit for brevity: `return len(nums) != len(set(nums))`. It's correct but always scans the full list even when a duplicate shows up early — mention the trade-off if you use it.
...
- Hash maps give average O(1) insert/lookup by trading memory for time — always name this trade-off in interviews.
- Chaining and open addressing are the two collision-resolution strategies; Python's dict uses open addressing but you reason about it as "average O(1), worst case O(n)."
