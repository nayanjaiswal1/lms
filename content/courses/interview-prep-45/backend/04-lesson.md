...
Without an index, PostgreSQL does a **sequential scan**: read every page of the table, check every row against the `WHERE` clause. For a small table this is often *faster* than an index lookup (see selectivity below) — that's the nuance interviewers are listening for.
...
**Selectivity** = distinct values / total rows. High selectivity (e.g. `customer_id`, near-unique) means an index prunes most of the table per lookup — huge win. Low selectivity (e.g. `status` with 4 possible values, or a boolean flag) means an index lookup still has to fetch a large fraction of the table's rows, and each of those is a random-access read from the heap — often slower than one sequential scan that reads pages in order.
...
Rule of thumb interviewers want to hear: **an index pays off when a query returns a small fraction of the table.** If a query typically returns >10-15% of rows, a sequential scan is usually cheaper — the planner knows this and will ignore your index (as seen above with `status`), which is *correct* behavior, not a bug.
...
- **`cost=A..B`** — planner's estimate, in arbitrary units, not milliseconds. `A` = startup cost, `B` = total cost.
- **`actual time=A..B`** — real measured milliseconds, only present with `ANALYZE` (which actually *runs* the query — be careful running it on a production `DELETE`/`UPDATE` without wrapping in a transaction you roll back).
...
**Column order in a composite index matters** — `(customer_id, status)` supports queries filtering on `customer_id` alone or `customer_id + status`, but not `status` alone (the index can't be used to skip to arbitrary status values without a leading `customer_id` predicate). This is the classic "leftmost prefix" rule.

## When NOT to use an index

- Low-selectivity columns queried alone (booleans, small enums) — sequential scan wins, as shown above.
- Small tables — a full table fits in a few pages; index overhead exceeds the scan cost.
...
- Wide/large text columns without a partial or expression index — indexing a whole `TEXT` column bloats the index; consider a `GIN`/trigram index or index only a prefix instead.

## Key takeaways

- B-tree indexes turn `O(n)` sequential scans into `O(log n)` tree descents, but the planner only uses one when it's actually cheaper.
- Selectivity (distinct values / total rows) is the number that determines whether an index helps — low-selectivity columns queried alone often get ignored by the planner, correctly.
