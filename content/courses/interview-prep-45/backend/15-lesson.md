...
- **Isolation**: concurrent transactions behave as if they ran one after another, to a degree controlled by the isolation level. This is the part interviewers actually probe, covered below.
...
Interview trap: candidates recite the acronym but can't say which letter Postgres' isolation levels actually govern. It's **I**: atomicity and durability are unconditional, consistency is enforced by constraints, and isolation is the only one that's *tunable*.
...
Postgres' MVCC keeps multiple versions of each row (tuples), each tagged with the transaction ID that created it. A reader only ever sees versions committed before its snapshot was taken. An uncommitted version is invisible to everyone but the transaction that wrote it. This is why Postgres can skip `READ UNCOMMITTED` entirely: there's no cheap "read whatever's there" mode to offer, because MVCC makes dirty reads structurally impossible.

## Savepoints: partial rollback inside a transaction

A savepoint lets you undo part of a transaction without abandoning the whole thing. It's useful for "try this operation, and if it fails, continue without it."
...
**What is ACID?** See the definitions above, and be ready to say which guarantee handles which failure: a crash maps to durability, concurrent writers to isolation, a constraint violation to consistency, and a partial failure to atomicity.
