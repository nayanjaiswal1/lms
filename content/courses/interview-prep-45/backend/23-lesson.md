...
A dependency is just a callable (function, or a class with `__call__`) that FastAPI calls for you before your endpoint runs, and whose return value it injects as an argument. Dependencies can themselves depend on other dependencies, and FastAPI resolves the whole graph per request.
...
- Nothing (`Depends()` with no argument, in newer FastAPI), which infers the callable from the annotated type.
...
For caching *across* requests (not just within one), don't rely on FastAPI's per-request cache. Use `functools.lru_cache` on the dependency itself instead, which is the standard pattern for expensive, request-independent setup like loading settings from environment variables once per process:
...
`use_cache=False` on `Depends(..., use_cache=False)` is the opposite override. It forces a dependency to re-run even if already resolved earlier in the same request, useful for a dependency with side effects you deliberately want repeated (rare, but interviewers may ask when you'd need it).
