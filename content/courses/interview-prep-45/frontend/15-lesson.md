...
- **Stored XSS** — attacker's payload is saved server-side (a comment, a username) and served to every viewer.
- **Reflected XSS** — payload comes from the URL/query string and is echoed back into the page immediately.
- **DOM-based XSS** — client-side JS takes untrusted data and writes it into the DOM without a server round-trip.

React protects you by default: JSX escapes everything you render as `{value}`. This is safe even if `value` is `"<img src=x onerror=alert(1)>"` — React renders it as text, not markup.
...
Defenses, from most to least common in modern stacks:

1. **`SameSite` cookies.** Set auth cookies with `SameSite=Lax` or `SameSite=Strict`. `Lax` (the browser default today) blocks cookies on cross-site POST requests but still allows top-level navigation — it stops the classic auto-submit form attack.
2. **CSRF tokens.** Server issues a random token embedded in the page (a hidden field or a meta tag); every mutating request must echo it back in a header. An attacker's cross-origin form can't read that token because of the same-origin policy.
...
No — CSRF exploits the browser's *automatic* credential attachment (cookies). If your auth token lives in JS memory/localStorage and you attach it manually via an `Authorization` header, a forged cross-site form can't replicate that, because it can't read your localStorage or set custom headers. That said, localStorage tokens are then exposed to XSS instead — it's a trade-off, not a free win.
...
It protects *users*, not servers. It stops a malicious website from reading another site's authenticated API responses in the victim's browser. It does nothing to stop direct server-to-server or curl requests — those never enforce CORS, because CORS is implemented by browsers, not by the HTTP protocol.
...
The safest common pattern for SPAs: short-lived JWT access token kept in memory (React state, not localStorage), long-lived refresh token in an `httpOnly`, `Secure`, `SameSite=Strict` cookie. This limits XSS blast radius (a stolen access token expires in minutes) and avoids CSRF (refresh cookie isn't readable by JS, and the endpoint is protected by `SameSite`).
...
- `default-src 'self'` — baseline: only load resources from your own origin.
...
- `frame-ancestors 'none'` blocks your site from being embedded in an `<iframe>` elsewhere, which is the standard clickjacking defense.
...
- React escapes JSX text by default; XSS reappears through `dangerouslySetInnerHTML`, unvalidated `href`/`src`, and manual DOM writes — sanitize with DOMPurify and allow-list URL protocols.
- CSRF exploits automatic cookie attachment; fix with `SameSite` cookies, CSRF tokens, or manual `Authorization` headers (which cross-site forms can't forge).
- CORS protects the *user's browser* from malicious frontends reading cross-origin responses; it is not a server-side access control and doesn't stop direct API calls.
