...
- [ ] Implement `myPromise`'s `resolve`/`reject`/`then` from memory, without looking at the reference implementation
- [ ] Be able to explain: why closures replace `this` in this implementation
- [ ] Be able to explain: how `resolve` unwraps a thenable passed into it
- [ ] Be able to explain: why `then` must always be asynchronous, even for an already-settled promise
- [ ] Implement `myPromise.all` and `myPromise.race` and trace through why each settles when it does
- [ ] Be able to explain: the connection between `flush()`/`queueMicrotask` here and the microtask queue from Day 3
