...
- [ ] Implement `myCall`, `myApply`, and `myBind` from memory, without looking at the reference implementation
- [ ] Be able to explain: why `myBind` is implemented using `apply` internally
- [ ] Be able to explain: why a `Symbol` key is used instead of a plain string key in `myCall`/`myApply`
- [ ] Be able to explain: what happens when `call`/`apply`/`bind` are used on an arrow function
- [ ] Be able to explain: `in` vs `hasOwnProperty` vs `Object.keys().includes()`
- [ ] Be able to explain: why `if (obj.key)` is the wrong way to check property existence
