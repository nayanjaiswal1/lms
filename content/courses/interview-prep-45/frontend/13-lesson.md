...
RTL exposes many ways to find elements; they're ranked by how closely they match how a real user (including assistive technology) finds things. Use this order:
...
4. **`getByTestId`** — last resort, only when nothing else identifies the element (e.g., a decorative element with no accessible name).
...
**Interview detail:** `getBy*` throws if not found (use for assertions that something exists). `queryBy*` returns `null` instead of throwing (use to assert something is *absent*). `findBy*` is async and retries until timeout (use when waiting for something to appear after an async action).
...
Note `userEvent` over `fireEvent`: `userEvent` simulates real user interaction sequences (focus, keydown, keypress, keyup, input events for each character typed) while `fireEvent` dispatches a single raw DOM event. `userEvent` catches bugs `fireEvent` can't — e.g., a component that only works because you skipped the focus event.
...
An integration test exercises multiple components together through a realistic sequence of user actions — closer to an end-to-end test but still running in the fast, mocked-network jsdom environment rather than a real browser.
...
This test doesn't care how cart state is stored (Context, Redux, Zustand) — it only cares that clicking "add to cart" eventually leads to a confirmation screen. That's exactly the resilience-to-refactor RTL's philosophy is aiming for.
...
- **Coverage as a floor, not a target.** Use it to find *completely untested* code paths (a branch nobody ever hit), not as a quality score to maximize.
- **Test the risky paths deliberately** — validation logic, error states, edge cases (empty list, network failure, race conditions) — rather than chasing percentage on straightforward render paths.
- **Mutation testing** (tools like Stryker) is the more honest signal: it mutates your source code (flips a `<` to `<=`, deletes a line) and checks whether any test fails. Tests that still pass after a mutation aren't actually verifying that code — this is worth mentioning if an interviewer pushes on "how do you know your coverage number means anything."

The interview-safe answer when asked "what coverage percentage do you target": there's no universal number — critical business logic (payments, auth, data mutations) should be close to fully covered with meaningful assertions; low-risk presentational code doesn't need the same investment. Chasing a blanket 100% target usually produces exactly the kind of hollow tests shown above.

## Key takeaways

- RTL's core principle: test what the user sees and does, not component internals — this makes tests survive refactors that don't change behavior.
- Query priority is `getByRole` > `getByLabelText` > `getByText` > `getByTestId`; `getBy*` throws (assert existence), `queryBy*` returns null (assert absence), `findBy*` is async (wait for appearance).
- Prefer `userEvent` over `fireEvent` — it simulates the full realistic event sequence instead of a single synthetic event.
