...
Core idea: split a large frontend into independently deployable pieces owned by different teams — an organizational scaling solution more than a technical one.
...
- `React.memo` skips a re-render if props are shallow-equal; it does nothing if you pass a new object/array/function literal each render — pair it with `useMemo`/`useCallback` on the parent for those values.
