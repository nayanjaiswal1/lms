...
- Re-render cascades from parent to child by default; `React.memo`'s shallow prop comparison is defeated by inline function/object literals passed as props.
- Hook order must be identical across renders because hooks are addressed by call-order index, not by name.
