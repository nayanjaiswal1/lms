...
--
-- Squashes migrations 001-025 (the prior 001_baseline through
-- 025_full_schema_consolidation, the ~181-table -> ~143-table architectural
-- consolidation) into a single baseline. Generated by replaying every prior
-- migration in order against a scratch database and pg_dump'ing the
-- resulting schema + reference data (pg_dump --inserts, so it runs as plain
-- SQL through a single Exec, no COPY/psql metacommands -- required by the Go
-- migration runner in db/migrate.go, which executes each file as one
-- pool.Exec call). The dump's \restrict/\unrestrict psql-only metacommands
-- were stripped.
--
-- Rationale for every table merge/drop/rename in this consolidation lives in
-- docs/schema-refactor-decisions.md, not here. Next migration is 002_*.
-- ══════════════════════════════════════════════════════════════════════════
...
CREATE TABLE public.comments (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    deleted_at timestamp with time zone,
    CONSTRAINT comments_subject_type_check CHECK ((subject_type = ANY (ARRAY['wiki_page'::text, 'interview_exp_qna'::text])))
);
...
CREATE TABLE public.coupons (
    ...
    CONSTRAINT coupons_discount_value_check CHECK ((((discount_type = 'percent'::text) AND ((discount_value >= 1) AND (discount_value <= 100))) OR ((discount_type = 'fixed'::text) AND (discount_value > 0)))),
...
CREATE TABLE public.courses (
    ...
    owner_id uuid,
    certificate_threshold_percent numeric,
...
CREATE TABLE public.feedback (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    kind text DEFAULT 'rating'::text NOT NULL,
    CONSTRAINT feedback_rated_or_skipped CHECK (((rating IS NOT NULL) OR (skipped_at IS NOT NULL))),
    CONSTRAINT feedback_rating_check CHECK (((rating >= 1) AND (rating <= 5))),
    CONSTRAINT feedback_subject_type_check CHECK ((subject_type = ANY (ARRAY['course'::text, 'assessment'::text, 'lab'::text, 'mentor'::text, 'mentor_session'::text, 'experience_subject'::text])))
);
...
CREATE TABLE public.habits (
    ...
    CONSTRAINT habits_cadence_check CHECK ((cadence = ANY (ARRAY['daily'::text, 'weekly'::text, 'monthly'::text]))),
...
CREATE TABLE public.mentor_sessions (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    notes text,
    notes_visible_to_student boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.org_settings (
    ...
    ai_connector jsonb DEFAULT '{}'::jsonb NOT NULL,
...
CREATE TABLE public.purchases (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    receipt_number text,
    product_type text DEFAULT 'course'::text NOT NULL,
    pack_id uuid,
    granted jsonb DEFAULT '{}'::jsonb NOT NULL,
...
CREATE TABLE public.session_credit_ledger (
    ...
    CONSTRAINT session_credit_ledger_reason_chk CHECK ((reason = ANY (ARRAY['purchase'::text, 'admin_grant'::text, 'admin_revoke'::text, 'booking'::text, 'cancellation_refund'::text])))
);
...
CREATE TABLE public.user_profiles (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    mentor_verified boolean DEFAULT false NOT NULL,
    mentor_verified_at timestamp with time zone,
    mentor_verified_by uuid,
    linkedin_url text,
    github_url text,
    portfolio_url text,
    whatnow_energy text,
...
INSERT INTO public.role_permissions VALUES ('11111111-1111-1111-1111-000000000003', '0bbf117d-6a58-40f9-b8bc-86a76a0c25b4');
INSERT INTO public.role_permissions VALUES ('11111111-1111-1111-1111-000000000004', '0bbf117d-6a58-40f9-b8bc-86a76a0c25b4');
