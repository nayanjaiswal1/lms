...
func (r *Repo) GetProfile(ctx context.Context, userID string) (*Profile, error) {
	const q = `
		SELECT
			...
			p.language, p.timezone, p.weekly_goal_hrs, p.default_landing_page,
			p.notifications,
	...
	err := r.pool.QueryRow(ctx, q, userID).Scan(
		...
		&prof.Language, &prof.Timezone, &prof.WeeklyGoalHrs, &prof.DefaultLandingPage,
		&notifRaw,
...
func (r *Repo) UpsertProfile(ctx context.Context, tx pgx.Tx, userID string, input UpdateProfileInput) error {
	...
	const upsertQ = `
		...
		ON CONFLICT (user_id) DO UPDATE SET
			...
			default_landing_page     = COALESCE($12, user_profiles.default_landing_page),
			notifications            = COALESCE($13, user_profiles.notifications),
			display_name             = COALESCE($14, user_profiles.display_name),
			bio                      = COALESCE($15, user_profiles.bio),
			public_enabled           = COALESCE($16, user_profiles.public_enabled),
			show_skills              = COALESCE($17, user_profiles.show_skills),
