...
// srs, interviewprep); this package owns no content, progress, or event data
// of its own beyond connection/token bookkeeping.
type Router struct {
	cfg              *config.Config
	repo             *Repo
...
// interviewPrepSvc are the exact same instances the rest of the API router
// shares, so MCP tool calls are authorized by the identical rules — and
// enforce the identical daily rate limit, in interviewPrepSvc's case — as the
// student-facing API.
func New(cfg *config.Config, pool *pgxpool.Pool, coursesRepo *courses.Repo, coursesSvc *courses.Service, calendarSvc *calendar.Service, mistakesRepo *mistakes.Repo, mistakesSvc *mistakes.Service, srsRepo *srs.Repo, interviewPrepSvc *interviewprep.Service) *Router {
	return &Router{
		...
		interviewPrepSvc: interviewPrepSvc,
	}
}
