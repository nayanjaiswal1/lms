...
| **Capabilities creep** | The capability set is decided entirely by operator config (`LABS_NESTED_DOCKER_IMAGES` / `LABS_NESTED_DOCKER_RUNTIME`), never by anything instructor-authored (not the `environment` string, not a per-lab column) — an image not on that allowlist gets zero added capabilities no matter what a lab's frontmatter claims. See "Nested Docker labs" below for the one image class this applies to. |
...
One lab image class, `mindforge/lab-docker:27`, needs to run a real `docker build`/`docker run` inside the student's own sandbox — teaching Docker itself requires Docker *inside* the lab container. Every other lab's isolation model (`--cap-drop ALL`, no added capabilities, no `--privileged`) makes that impossible by design, so this is a deliberate, narrowly-scoped exception, not a relaxation of the default:

- **Off by default, two independent switches must both be set.** Operator config: `LABS_NESTED_DOCKER_IMAGES` (comma-separated image allowlist; empty = feature does not exist on this deploy) and optionally `LABS_NESTED_DOCKER_RUNTIME=sysbox-runc` / `LABS_NESTED_DOCKER_RUNTIME_CLASS` (Kubernetes). Org config: the same image must also be in that org's `lab_org_config.allowed_images` — an operator enabling the image platform-wide does not itself grant any org access to it.
- **What a nested-image container actually gets** (`internal/labs/container.go` `buildRunArgs`/`startNamed`, verified interactively against `mindforge/lab-docker` — `docker:27-dind-rootless`): on a host with `sysbox-runc` installed, `--cap-drop ALL --runtime sysbox-runc` with no added capabilities at all (strictly better; set `LABS_NESTED_DOCKER_RUNTIME=sysbox-runc` to use it). Otherwise, every attempt starts with the scoped grant: `--cap-drop ALL --cap-add SYS_ADMIN --cap-add SETUID --cap-add SETGID --device /dev/fuse --device /dev/net/tun --security-opt seccomp=unconfined --security-opt apparmor=unconfined`. `SETUID`/`SETGID` are both required, not just `SYS_ADMIN` — rootless dockerd's startup calls the setuid-root `newuidmap`/`newgidmap` helpers to build its user-namespace UID/GID map, and those fail with "operation not permitted" without them; `/dev/net/tun` is needed because rootlesskit's default `vpnkit` network driver creates a tap interface at startup and dockerd never reaches a running state without it. Resource limits step up to 2 CPU / 1.5 GB (`NestedContainerCPU`/`NestedContainerMemoryMB`) — a nested dockerd plus a student `docker build` does not fit in the default 1 CPU / 512 MB.
...
- [x] `lab-docker` nested-Docker gating (operator `LABS_NESTED_DOCKER_IMAGES` allowlist + org `lab_org_config.allowed_images`, both required, off by default — see "Nested Docker labs" above)
