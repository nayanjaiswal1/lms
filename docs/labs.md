...
| **Malicious verification script** | Instructors are org-admins (trusted). Verification runs inside the user container — no host mount, no bare `--privileged`. Worst case for an ordinary lab: student's own container is affected. For a nested-Docker lab (see "Nested Docker labs" below), the container holds real elevated capabilities — worst case is host-level, which is why that image class is operator- and org-gated separately from ordinary trust-the-instructor reasoning. |
...
| Case | Handling |
|---|---|
| **Container escape via runC / kernel exploit** | Every container starts with `--cap-drop ALL`. Ordinary labs add nothing back (`--security-opt no-new-privileges`, no `--cap-add`). Nested-Docker labs are the one exception — see "Nested Docker labs" below for exactly what they add and why that's still treated as a live, unresolved risk rather than a solved one. |
| **Docker socket mounted into container** | The Docker socket (`/var/run/docker.sock`) must never be mounted into any *lab* container. This gives the container full host Docker API access — game over. Blocked at container creation time; any image that specifies a socket mount in its entrypoint is rejected. (The backend *service* container itself does mount the host socket, to drive `docker run`/`exec` for lab provisioning — that mount is on the trusted backend, not on any student-reachable container.) |
...
| **Capabilities creep** | The capability set is decided entirely by operator config (`LABS_IMAGE_PROFILES` / `LABS_NESTED_DOCKER_RUNTIME`), never by anything instructor-authored (not the `environment` string, not a per-lab column) — an image with no profile mapping gets zero added capabilities no matter what a lab's frontmatter claims. See "Nested Docker labs" below for the one image class this applies to. |
| **Namespace collision between containers** | All lab containers are on the shared `mindforge-labs` bridge network (nested-Docker containers use a separate `mindforge-labs-dind` bridge instead — see below); no per-session subnet allocation exists today. Do not cite `--userns-remap` as implemented — it is not currently configured on either runtime. |
...
- **No bare `--privileged` mode for ordinary labs, ever.** Nested-Docker labs get the scoped `--cap-add`/rootless-dind grant first, always — see "Nested Docker labs" below for the one narrow, self-detecting exception to that (a `--privileged` retry, only reached when the scoped grant demonstrably fails), plus the Kubernetes `RuntimeClass` alternative. Both paths are gated by operator config + org allowlist, off by default.
...
- **Image allowlist, actually enforced**: `Service.StartSession` denies unless the lab's `environment` is in the requesting org's `lab_org_config.allowed_images` — empty means "no restriction" for an ordinary image, but a nested-Docker image (below) is never covered by that default and always needs an explicit entry.
...
### Nested Docker labs (Docker-in-Docker)

One lab image class needs to run a real `docker build`/`docker run` inside the student's own sandbox — teaching Docker itself requires Docker *inside* the lab container. Every other lab's isolation model (`--cap-drop ALL`, no added capabilities, no `--privileged`) makes that impossible by design, so this is a deliberate, narrowly-scoped exception, not a relaxation of the default. **Two image variants exist and are not interchangeable** — which one an operator builds and deploys must match `LABS_NESTED_DOCKER_RUNTIME` on that host, since that env var picks the mechanism for every nested-docker image platform-wide (see below), not per-image:
...
- **Production node-pool isolation, not just RuntimeClass isolation**: `k8s/base/runtimeclass-sysbox.yaml` declares `scheduling.nodeSelector: mindforge.io/sysbox=true` (so a Pod using it can only land on a node that actually has sysbox-runc registered with containerd — an unlabeled node fails scheduling, never silently drops to an unisolated runtime) and tolerates a `mindforge.io/sysbox-only` taint. Neither the label nor the taint exist on any real node until an operator applies them — provisioning a real cluster for this means: install sysbox-ce + register it in containerd on a dedicated node pool (`docs.k3s.io/blog/2025/09/27/k3s-sysbox` for the containerd config), `kubectl label` those nodes `mindforge.io/sysbox=true`, and `kubectl taint` them `mindforge.io/sysbox-only=true:NoSchedule` so untrusted student `docker build`/`docker run` workloads never share a kernel with anything else in the cluster — sysbox's own isolation is real but not a substitute for this, the same way a chroot isn't a substitute for a VM. `scripts/setup-sysbox-local.sh` only does the label (a single-node cluster has no separate pool to taint) — see `docs/local-k3s-dev.md` "Known local-only limitations".
...
| **Pre-warm pool & validation container orphans** | Pool containers use the `mindforge-warm-*` prefix and validation containers `mindforge-validate-*`. `lab.cleanup_containers` reconciles all three prefixes (`lab`/`warm`/`validate`) against live state so a crashed node never leaks pooled or validation containers. |
...
- [x] `lab-docker` nested-Docker gating (operator `LABS_IMAGE_PROFILES` → `nested-docker` profile + org `lab_org_config.allowed_images`, both required, off by default — see "Nested Docker labs" above)
